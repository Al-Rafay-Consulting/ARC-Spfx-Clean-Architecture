...
class EmployeeService {
    ...
    async getCustomers() {
        const response = await this.basePnpService.getAll(this._listName, [MODULE_ENUMS.SP.COLUMS.ALL]);
        return response;
    }
    ...
    async updateCustomer(itemId: number, payload: ICreatePayload) {
        const response = await this.basePnpService.update(this._listName, itemId, payload);
        return response;
    }
}
